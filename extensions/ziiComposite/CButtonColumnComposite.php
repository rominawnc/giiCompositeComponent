 ...
 * @since 1.1
 */
class CButtonColumnComposite extends CGridColumn
{
	/**
...
EOD;
	...
	protected function renderDataCellContent($row,$data)
	{
		...
		if (is_array($data->primaryKey))
			$this->initButtonsWithCompositeKey();	

		foreach($this->buttons as $id=>$button)
		{
			
			if (is_array($data->primaryKey)){
				$button=$this->overrideButtonUrl($button,$id);
			}
	...
	protected function initButtonsWithCompositeKey(){		
		$this->viewButtonUrl='Yii::app()->controller->createUrl("view",$data->primaryKey)';
		$this->updateButtonUrl='Yii::app()->controller->createUrl("update",$data->primaryKey)';
	 	$this->deleteButtonUrl='Yii::app()->controller->createUrl("delete",$data->primaryKey)';
	}
	protected function overrideButtonUrl($button,$id){
